...
@build:
  bun slidev build --base=/dealroom-2024-polars
...
[private]
  ...
  #! /usr/bin/env zsh
  if ! gh repo list baggiponte | grep --quiet "/\bdealroom-2024-polars\b"; then
    while true; do
      ...
      case $REPLY in
        [Yy])
          ...
          case $PRIVATE in
            [Yy])
              gh repo create baggiponte/dealroom-2024-polars --private --source=.
              ;;
            [Nn])
              gh repo create baggiponte/dealroom-2024-polars --public --source=.
              ;;
            *)
    ...
    while true; do
      ...
      case $REPLY in
        [Yy])
          git remote add origin git@github.com:baggiponte/dealroom-2024-polars
          break
          ;;
        [Nn])
          break
